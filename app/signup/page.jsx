...
export default function Signup() {
  ...
  const handleAuth = (e) => {
    ...
    if (nameInput) {
      localStorage.setItem('workspaceName', nameInput);
    }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', color: '#381a11' }}>
      <div style={{ width: '100%', maxWidth: '400px', padding: '40px', background: 'rgba(255, 255, 255, 0.7)', backdropFilter: 'blur(20px)', borderRadius: '16px', border: '1px solid rgba(242, 85, 44, 0.15)', boxShadow: '0 8px 32px rgba(92, 38, 20, 0.08)' }}>
        <h2 style={{ margin: '0 0 24px 0', textAlign: 'center' }}>Create an account</h2>
        <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', color: '#704133', fontWeight: 500 }}>Name</label>
            <input type="text" required style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.9)', border: '1px solid rgba(242, 85, 44, 0.2)', borderRadius: '8px', color: '#381a11', boxSizing: 'border-box', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = 'rgba(242, 85, 44, 0.5)'} onBlur={(e) => e.target.style.borderColor = 'rgba(242, 85, 44, 0.2)'} />
          </div>
          <div>
